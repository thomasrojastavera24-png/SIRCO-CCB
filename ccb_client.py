...
class CCBClient:
    ...
    def _lanzar_chromium(p):
        """
        Intenta lanzar Chromium con tres estrategias en orden:
        1. Ejecutable del sistema (/usr/bin/chromium) — Streamlit Cloud con packages.txt
        2. Playwright gestionado (playwright install chromium) — entorno local
        3. Lanza excepción con el error original acumulado
        """
        errores = []

        # 1. Ejecutable del sistema
        for exe in ("/usr/bin/chromium", "/usr/bin/chromium-browser"):
            try:
                ...
                errores.append(f"executable_path={exe}: {e}")

        # 2. Playwright gestionado (sin ruta explícita)
        try:
            return p.chromium.launch(
                headless=True,
                args=["--no-sandbox", "--disable-dev-shm-usage"],
            ...
            errores.append(f"playwright-managed: {e}")

        raise RuntimeError("Chromium no disponible. Intentos:\n" + "\n".join(errores))
    ...
    def __enter__(self):
        ...
        try:
            ...
            self._browser = self._lanzar_chromium(self._playwright)
